...
shinyUI(fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      sliderInput('nClustersSimulate', 'Clusters to Simulate', 5, min = 2, max = 6, step = 1),
      ...
      sliderInput('kClustersFit', 'Clusters to Fit', 5, min = 2, max = 6, step = 1),
      "Note: The choice of symbol indicates which cluster the point was a simulated to be a member of, ",
      "and the choice of color and separating line segments ",
      "indicate which cluster the K-Means algorithm assigned the point to."
    ),
    mainPanel(
